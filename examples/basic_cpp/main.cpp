...
int main() {
    ...
    console->set_formatter(
        std::make_unique<br_logger::PatternFormatter>(
            "[%D %T.%e] [%L] [tid:%t] [%f:%#] %g %m"));
    ...
    file_sink->set_formatter(std::make_unique<br_logger::JsonFormatter>());
    ...
    logger.add_sink(std::move(cb_sink));

    // --- Configuration ---

    logger.set_level(br_logger::LogLevel::Trace);
    br_logger::LogContext::instance().set_process_name("basic_example");
    br_logger::LogContext::instance().set_app_version("1.0.0");
    br_logger::LogContext::set_thread_name("main");

    // Global tag visible to all threads
    br_logger::LogContext::instance().set_global_tag("env", "dev");
    ...
    auto worker = [](int id) {
        ...
        br_logger::LogContext::set_thread_name(name);
