...
TEST(MPSCRingBuffer, SinglePushPop) {
    ...
    EXPECT_TRUE(buffer.try_push(in));
...
TEST(MPSCRingBuffer, PushPopMultiple) {
    ...
    for (uint32_t i = 0; i < 10; ++i) {
        items.push_back({2, i});
        EXPECT_TRUE(buffer.try_push(items.back()));
    }
...
TEST(MPSCRingBuffer, EmptyPopFails) {
    MPSCRingBuffer<TestItem, 8> buffer;
    TestItem out{};
    EXPECT_FALSE(buffer.try_pop(out));
}

TEST(MPSCRingBuffer, FullPushFails) {
    MPSCRingBuffer<TestItem, 4> buffer;
    EXPECT_TRUE(buffer.try_push({1, 1}));
    EXPECT_TRUE(buffer.try_push({1, 2}));
    EXPECT_TRUE(buffer.try_push({1, 3}));
    EXPECT_TRUE(buffer.try_push({1, 4}));
    EXPECT_FALSE(buffer.try_push({1, 5}));
}
...
TEST(MPSCRingBuffer, EmptyPredicate) {
    ...
    EXPECT_TRUE(buffer.try_push({0, 0}));
...
TEST(MPSCRingBuffer, WrapAround) {
    ...
    for (uint32_t round = 0; round < 5; ++round) {
        for (uint32_t i = 0; i < 8; ++i) {
            EXPECT_TRUE(buffer.try_push({round, i}));
        }
        for (uint32_t i = 0; i < 8; ++i) {
            TestItem out{};
            EXPECT_TRUE(buffer.try_pop(out));
...
TEST(MPSCRingBuffer, MultiProducerSingleConsumer) {
    ...
    for (uint32_t producer = 0; producer < kProducerCount; ++producer) {
        producers.emplace_back([producer, &buffer]() {
