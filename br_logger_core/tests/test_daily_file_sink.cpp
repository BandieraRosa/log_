...
protected:
    ...
    void TearDown() override {
        remove_directory_recursive(test_dir_);
    }
...
TEST_F(DailyFileSinkTest, FilenameFormatMatchesPattern) {
    ...
    std::string filename = sink.make_filename(t);
...
TEST_F(DailyFileSinkTest, MakeFilenameUTCvsLocal) {
    ...
    std::string fn_utc = sink_utc.make_filename(t);
    std::string fn_local = sink_local.make_filename(t);
...
TEST_F(DailyFileSinkTest, MakeFilenameDifferentDays) {
    ...
    std::tm tm1{};
    tm1.tm_year = 126; tm1.tm_mon = 0; tm1.tm_mday = 1; tm1.tm_hour = 12;
    std::time_t t1 = ::timegm(&tm1);

    std::tm tm2{};
    tm2.tm_year = 126; tm2.tm_mon = 0; tm2.tm_mday = 2; tm2.tm_hour = 12;
    std::time_t t2 = ::timegm(&tm2);

    std::string fn1 = sink.make_filename(t1);
    std::string fn2 = sink.make_filename(t2);
