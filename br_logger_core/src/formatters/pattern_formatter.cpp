...
void br_logger::PatternFormatter::CompilePattern()
{
  ops_.clear();
  std::string literal;
  ...
  for (size_t i = 0; i < pattern_.size(); ++i)
  {
    ...
    switch (next)
    {
      case 'D':
        flush_literal();
        ops_.push_back({OpType::Date, {}});
        break;
      case 'T':
        flush_literal();
        ops_.push_back({OpType::Time, {}});
        break;
      case 'e':
        flush_literal();
        ops_.push_back({OpType::Microseconds, {}});
        break;
      case 'L':
        flush_literal();
        ops_.push_back({OpType::LevelFull, {}});
        break;
      case 'l':
        flush_literal();
        ops_.push_back({OpType::LevelShort, {}});
        break;
      case 'f':
        flush_literal();
        ops_.push_back({OpType::FileName, {}});
        break;
      case 'F':
        flush_literal();
        ops_.push_back({OpType::FilePath, {}});
        break;
      case 'n':
        flush_literal();
        ops_.push_back({OpType::FuncName, {}});
        break;
      case 'N':
        flush_literal();
        ops_.push_back({OpType::PrettyFunc, {}});
        break;
      case '#':
        flush_literal();
        ops_.push_back({OpType::Line, {}});
        break;
      case 't':
        flush_literal();
        ops_.push_back({OpType::ThreadId, {}});
        break;
      case 'P':
        flush_literal();
        ops_.push_back({OpType::ProcessId, {}});
        break;
      case 'k':
        flush_literal();
        ops_.push_back({OpType::ThreadName, {}});
        break;
      case 'q':
        flush_literal();
        ops_.push_back({OpType::SequenceId, {}});
        break;
      case 'g':
        flush_literal();
        ops_.push_back({OpType::Tags, {}});
        break;
      case 'm':
        flush_literal();
        ops_.push_back({OpType::Message, {}});
        break;
      case 'C':
        flush_literal();
        ops_.push_back({OpType::ColorStart, {}});
        break;
      case 'R':
        flush_literal();
        ops_.push_back({OpType::ColorReset, {}});
        break;
      case '%':
        literal.push_back('%');
        break;
...
size_t br_logger::PatternFormatter::Format(const LogEntry& entry, char* buf,
                                           size_t buf_size)
{
  if (buf_size == 0)
  {
  ...
  for (const auto& op : ops_)
  {
    ...
    switch (op.type)
    {
      ...
      case OpType::Time:
      {
        ...
        int n = std::snprintf(temp, sizeof(temp), "%02d:%02d:%02d", tm_val.tm_hour,
